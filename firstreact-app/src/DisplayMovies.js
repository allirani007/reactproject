...
export function DisplayMovies({
  ...
  return (
    <div className="col-4 mb-3">
      <img className="movie-img" src={poster} alt="Movie name" />
      <div>
        <h6>
          Movie Title: {name}
          <IconButton color="primary" aria-label="add to shopping cart">
            {hide ? (
              <ExpandLessIcon onClick={() => sethide(!hide)} />
            ) : (
              <ExpandMoreIcon onClick={() => sethide(!hide)} />
            )}
          </IconButton>
          <IconButton
            color="primary"
        ...
        {hide ? (
          <p>
            {summary}
            <br></br>
            <span style={styles} className="rr">
              {rating}
            </span>
          </p>
        ) : (
